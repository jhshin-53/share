...
SPAN_PX_PER_STEP = 30    # reduced px spacing for sampling along spans
...
OPTI_SANS_LINE = 3  # 라인을 몇개 쓸건지 여부. 0이면 다씀.
...
def img_process(num, start, total_len, imgfile):
	...
	if OPTI_SANS_LINE != 0 :
		spans_opti = []
		spans_opti.append(spans[len(spans)//5])
		spans_opti.append(spans[len(spans)//2])
		spans_opti.append(spans[len(spans) - len(spans)//10])
		spans = spans_opti
	
	# MaxJung
	# i = 0;
	# if OPTI_SANS_LINE != 0 :
	#     while(len(spans) > OPTI_SANS_LINE) :
	#         i = i+1;
	#         if(i%2 == 0) :
	#             spans.pop(len(spans)//3)
	#         else :
	#             spans.pop(len(spans)//2)
